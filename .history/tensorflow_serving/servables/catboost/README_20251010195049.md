...
For building and running on Windows:

- Ensure Bazel is installed and available in your PATH. If installed via WinGet, the path should be:
  ```plaintext
  %LOCALAPPDATA%\Microsoft\WinGet\bin\bazel.exe
  ```
- Use Git Bash or WSL for a Unix-like environment.
- Set up environment variables as needed for Bazel and TensorFlow Serving.
